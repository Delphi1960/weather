...
export default function MoonPhaseIcon({ moonPhase, sizeIcon }: phaseProps) {
  function GetMoonPhaseIcon(moonPhase: string) {
    ...
    if (phase === 0) return Icons.moon0; //новолунье
    if (phase > 0 && phase <= 4) return Icons.moon4;
    if (phase > 4 && phase <= 8) return Icons.moon8;
    if (phase > 8 && phase <= 12) return Icons.moon12;
    if (phase > 12 && phase <= 16) return Icons.moon16;
    if (phase > 16 && phase <= 25) return Icons.moon20;
    if (phase === 25) return Icons.moon25; //первая четверть
    ...
    if (phase > 41 && phase <= 49) return Icons.moon45;
    if (phase === 50) return Icons.moon50; //полнолуние
    ...
    if (phase > 66 && phase <= 75) return Icons.moon70;
    if (phase === 75) return Icons.moon75; //последняя четверть
    ...
    if (phase > 91 && phase <= 99) return Icons.moon95;
    // if (moonPhase === "back") return Icons.back;
  }
